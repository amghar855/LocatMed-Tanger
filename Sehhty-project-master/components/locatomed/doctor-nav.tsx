...
type DoctorTab = "dashboard" | "appointments" | "patients" | "consultations";
...
export default function DoctorNav({ userName = "Médecin", active }: Props) {
  const initials =
    userName
      .split(" ")
      .filter(Boolean)
    ...
    <>
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r bg-background md:flex md:flex-col">
        ...
        <div className="px-4 py-4">
          <div className="rounded-xl border border-teal-200/80 bg-teal-50/80 p-3">
            ...
            {NAV_LINKS.map(({ key, href, label, icon: Icon }) => (
              <Link
                ...
                className={cn(
                  ...
                  active === key
                    ? "bg-teal-600 text-white"
                    : "text-foreground hover:bg-teal-50 hover:text-teal-900"
                )}
              >
                <Icon className="h-4 w-4" />
                {label}
