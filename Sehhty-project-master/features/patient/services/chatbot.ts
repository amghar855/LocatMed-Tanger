...
export async function askPatientChatbot(question: string, _patientId: string): Promise<ChatbotResponse> {
  ...
  const text = (() => {
    ...
    return "Marhba bik f LOCATOMED. Nqdar n3awnk b reservation, recherche de medicaments, w decouverte dyal hopitaux f Tanger.";
  })();
...
export async function getPatientChatbotStarterContext(): Promise<ChatbotStarterContext> {
  ...
  return {
    welcomeMessage:
      "Bonjour, je suis votre assistant LOCATOMED. Posez une question sur la reservation, les hopitaux ou la disponibilite des medicaments.",
